...
namespace CKAN.CmdLine
{
    internal abstract class MainClass
    {
                                           ...
                                           GameInstanceManager manager)
        {
            ...
            try
            {
                return options switch
                {
                        ...
                        GuiOptions opts when Platform.IsWindows =>
                            Gui(manager, opts, args),
                    #endif
                    #if NET6_0_OR_GREATER && !WINDOWS
                        GuiOptions opts when !Platform.IsWindows =>
                            AvaloniaGui(manager, opts, args),
                    #endif
        ...
        #if NET6_0_OR_GREATER && !WINDOWS
        [ExcludeFromCodeCoverage]
        private static int AvaloniaGui(GameInstanceManager manager, GuiOptions options, string[] args)
        {
            CKAN.GUI.Avalonia.AvaloniaGui.Main_(
                args.Except(new string[] {"--verbose", "--debug", "--show-console", "--asroot"})
                    .ToArray(),
