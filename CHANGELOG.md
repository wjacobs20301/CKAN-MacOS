...
> **Note:** This changelog is inherited from the upstream [KSP-CKAN/CKAN][upstream]
> project, which is the source of essentially all functionality described
> below. Entries tagged `[macOS]` or `[GUI.Avalonia]` cover work done in
> this fork (the native macOS app, Avalonia GUI, universal binary,
> packaging, and data migration). All other entries are the work of the
> upstream CKAN authors — see the [contributor list][contributors].
>
...
Initial release of the macOS-focused fork of CKAN.

### Highlights

- Native macOS application built on Avalonia — no Terminal, no Mono
- Universal binary for Apple Silicon (arm64) and Intel (x86_64)
- Retina / high-DPI support
- In-app update checker that pings GitHub Releases and prompts to download new `.dmg` builds
- macOS-only codebase: removed WinForms GUI, Windows AutoUpdate helper, Debian/RPM packaging, Dockerfiles, and all non-`net8.0` targets
- Attribution and credit to the upstream [KSP-CKAN/CKAN][upstream] project, whose authors wrote essentially all of the mod manager itself

Historical changelog from the upstream project is preserved below for reference.
...
- **Native macOS app**: CKAN now ships a fully graphical, universal (arm64 + x86_64) macOS application built on Avalonia. The old Terminal/ConsoleUI launcher is gone — just double-click `CKAN.app`.

### Features

- [macOS] New native Avalonia-based CKAN.app replacing the Terminal/ConsoleUI launcher — high-DPI, universal binary, feature parity with the Windows GUI
- [GUI.Avalonia] New cross-platform GUI also available on Linux via `ckan gui`
...
- [Build] Coverage and more tests (#4383, #4396, #4408, #4420, #4422, #4423, #4425, #4426, #4429, #4435, #4439, #4440, #4441, #4443, #4444, #4446, #4447, #4448, #4454 by: HebaruSan)
